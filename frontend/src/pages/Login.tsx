...
const Login = () => {
  ...
  // ZMENA: Zjednodušili sme stav, už nepotrebujeme 'isAuthenticating'
  // Globálny 'loading' z App.tsx je postačujúci.

  useEffect(() => {
    // Akonáhle 'loading' skončí a 'user' existuje, presmeruj preč z loginu
    if (!loading && user) {
      navigate('/');
    }
  ...
  const handleSignIn = () => {
    setError(null);
    // ZMENA: Použijeme 'signInWithPopup'
    signInWithPopup(auth, googleProvider)
      .then((result) => {
        // Používateľ je prihlásený, 'onAuthStateChanged' v App.tsx
        // sa postará o aktualizáciu stavu a presmerovanie.
        // Pre istotu môžeme navigovať aj tu.
        navigate('/');
      })
      .catch((err) => {
        console.error("Authentication error:", err);
        // Zobrazíme chybu, ak používateľ zavrie okno alebo nastane chyba
        setError("Failed to sign in. Please try again.");
      });
  };

  // ZMENA: Zobrazujeme globálny loader z App.tsx, alebo loader z Login.tsx, ak sa user už načítal
  // Tento stav 'loading' (z App.tsx) je ten, ktorý vidíš zaseknutý.
  // Akonáhle sa zmení metóda, už sa nezasekne.
  if (loading) {
     return <Loader fullScreen={true} message="Loading app..." />;
  }
  
  // Ak sa aplikácia načítala, ale user ešte nie je (sme na /login),
  // zobrazíme prihlasovacie tlačidlo.
  if (!user) {
    return (
  ...
  // Ak sa toto deje (načítané, ale user existuje), App.tsx useEffect ho presmeruje
  return <Loader fullScreen={true} message="Redirecting..." />;
};
